...
#### localstorage 和 sessionStorage
   ...
   ###### MVVM
   + MVC
      + 什么是mvc全名是Model View Control 是模型 视图 控制器的缩写
      + 一种软件设计典范，用一种业务逻辑，数据，界面显示分离的方法组织代码
      + model是数据模型，view是视图或者说就是我们软件界面需要去展示的东西，controller是用来控制model的读取，存储已经如何在view上展示数据，更新数据的逻辑控制
   + MVC三者的关系
      + view传送指令到cotroller
      + controller完成业务逻辑后，要求model该百年状态
      + model将新的数据发送到view
      + 所有的通信都是单向的，
   + MVC的优点
      + 耦合性低
   ...
   + MVC的缺点
      + 不适合小型中等规模的应用程序
      + view和model之间不匹配
      + controler和model之间的界限不清，什么样的逻辑是界面逻辑，什么样的逻辑是业务逻辑，很难定义清楚，没有明确的定义
   + MVVM
      + 因为MVC不适合小型 中等规模的应用程序，随着h5的不断发展
         + 开发者在代码中大量调用相同的Dom api处理繁琐，操作冗余 使得代码难以维护
         + 大量dom操作使得页面渲染性能降低，加载速度变慢，影响用户体验
         + model变化，更新view  view变化同步到model 这样工作不仅繁琐，而且很难维护复杂多变的数据状态
         + MVP
            + m,v,p之间是双向通信
            + view和model是不通信的，presenter完全把model和view进行了分离
         + mvp的优点
      ...
      + 什么是MVVM
         + M(model)层： 模型，定义数据结构
         + C(controller)层： 实现业务逻辑，数据的增删改查
         + ViewModel 是视图view的模型，映射显示逻辑
      + mvvm之间的关系
         + view和model其实没有直接的联系，而是通过viewModel进行交互，model和viewModel之间是双向的，因此view的变化会影响model model的数据变化也会反映到view上
         + viewModel通过双向数据绑定把view和model层连接起来。
         + mvvm模式的优缺点
            + 低耦合
            + 可重用性
            + 可测试性
